...
const RACING_MODEL_V3_FUEL_CONTRACT_CANDIDATE_MANIFEST: &[u8] =
    b"pitgun.racing-v3-candidate:model:0.15.0:resolved-vehicle:per-segment-v1:aggregate-contact-v2:mechanical-controls-v1:aero-efficiency-v1:development-resolution-v1:transmission-resolution-v1:zero-downforce-v1:first-stint-tire-v1:power-based-fuel-mass-v1:compound-degradation-v1:engine-thermal-resolution-v2:per-competitor-components-v1:driver-traits-v1:driving-mode-v1:correction-workload-v1:correction-friction-budget-v1:catalog-driver-control-v1:driver-instruction-timeline-v1:catalog-fuel-contract-v1";
...
/// Returns the non-production identity of the catalog-governed fuel slice.
///
/// Candidate 0.15 preserves the 0.14 physics and timeline semantics while
/// moving the published initial load and consumption coefficients into one
/// immutable catalog resource.
#[must_use]
pub fn racing_model_v3_fuel_contract_candidate_identity() -> ArtifactIdentity {
    ArtifactIdentity {
        id: "pitgun.racing-v3-candidate"
            .parse()
            .expect("static Racing V3 candidate model id"),
        version: "0.15.0"
            .parse()
            .expect("static Racing V3 fuel-contract candidate model version"),
        digest: pitgun_contract::Digest::from_bytes(
            RACING_MODEL_V3_FUEL_CONTRACT_CANDIDATE_MANIFEST,
        ),
    }
}
...
pub fn racing_model_identity_for_version(version: &str) -> Result<ArtifactIdentity, String> {
    match version {
        ...
        "0.15.0" => Ok(racing_model_v3_fuel_contract_candidate_identity()),
        _ => Err(format!(
            "unsupported Racing model version {version:?}; expected 1.0.0, 2.0.0, 0.10.0, 0.11.0, 0.14.0 or 0.15.0"
        )),
    }
}
...
impl RacingWorkload {
    ...
    /// Creates the catalog-governed fuel-contract candidate pinned to release 1.9.
    #[must_use]
    pub fn v3_fuel_contract_with_catalog(catalog: RacingCatalogSnapshot) -> Self {
        Self {
    ...
    pub fn for_model(
        ...
        } else if *model == racing_model_v3_fuel_contract_candidate_identity() {
            Ok(Self::v3_fuel_contract_with_catalog(catalog))
        } else {
            Err(format!(
    ...
    fn execute(
        ...
        let output = match &self.catalog {
            Some(catalog) => {
                ...
                } else if self.model == racing_model_v3_fuel_contract_candidate_identity() {
                    run_race_with_catalog_and_v3_fuel_contract_candidate(
                        request,
                        catalog,
                        pitgun_racing_contract::RacingDriverInstructionTimelineV1 {
...
mod tests {
    ...
    fn model_version_selection_is_exact_and_fail_closed() {
        ...
        assert_eq!(
            racing_model_identity_for_version("0.15.0")
                .expect("supported V3 fuel-contract candidate"),
            racing_model_v3_fuel_contract_candidate_identity()
        );
    ...
    fn v3_candidate_identity_cannot_masquerade_as_a_published_model() {
        ...
        let fuel_contract = racing_model_v3_fuel_contract_candidate_identity();
        assert_eq!(fuel_contract.id.to_string(), "pitgun.racing-v3-candidate");
        assert_eq!(fuel_contract.version.to_string(), "0.15.0");
        assert_eq!(
            fuel_contract.digest.to_string(),
            "sha256:3038739c9059c12cf47feb4de6a3fd791d9f94290d9e83405a61bd966eea540f"
        );
        assert_ne!(fuel_contract, timeline);
    }
}
