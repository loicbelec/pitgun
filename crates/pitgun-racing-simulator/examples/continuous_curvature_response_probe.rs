//! Offline-only probe for the continuous curvature-response candidate.
...
use pitgun_racing_simulator::{
    CurvatureAeroResponse, RacingCatalogSnapshot, RunRaceInput, RunRaceRequest,
    SetupResponseDiagnosticsV1, TuningResponseV1, run_race_with_catalog_and_model_response,
};
use serde::Serialize;
...
fn run() -> Result<(), String> {
    ...
    if arguments.len() != 3 {
        return Err(
            "usage: continuous_curvature_response_probe SCENARIO_JSON TUNING_RESPONSE_JSON SEED"
                .to_string(),
        );
    }
    ...
    let experimental_execution_id = canonical_json_digest(&ExperimentalIdentity {
        schema_version: "pitgun.racing-continuous-curvature-response-probe/v1",
        scenario: &scenario,
    ...
    let output = run_race_with_catalog_and_model_response(
        RunRaceRequest {
            era: Some(input.era),
            hz: Some(input.hz),
            input,
