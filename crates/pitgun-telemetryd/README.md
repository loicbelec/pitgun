...
Telemetry ingestion daemon that sits behind Caddy (`telemetry.pitgun.com` -> `127.0.0.1:8080`). It accepts EventBatch payloads over HTTP and WebSocket, queues them, and writes append-only NDJSON for later processing.

## Endpoints
- `GET /health` – readiness check, returns 200.
- `POST /beacon` – accepts JSON EventBatch, returns 202 (fallback for `navigator.sendBeacon`).
- `GET /ws` – WebSocket server; accepts JSON text frames (required) and protobuf binary frames (feature `protobuf`).
...
curl -X POST http://127.0.0.1:8080/beacon \
  ...
  -d '{"events":[{"channel":"demo","ts_ns":"1710000000000000000","value":1.23}],"end_of_stream":false}'
```
...
Then paste a batch:
```json
{"events":[{"channel":"demo","ts_ns":"1710000000000000000","value":3.14}],"end_of_stream":false}
```
