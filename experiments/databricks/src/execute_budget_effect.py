...
catalog_name = validated_identifier("catalog_name", dbutils.widgets.get("catalog_name"))
calibration_schema = validated_identifier(
...
with tracking_context as tracking_run:
        ...
        run_rows.append(
            {
                ...
                "configuration_id": result["configuration_id"] if result else "unresolved",
                ...
                "canonical_result_digest": (
                    adapter_result["canonical_result_digest"] if adapter_result else None
                ),
                "source_git_revision": source_git_revision,
                "circuit_id": entry["circuit_id"],
                "era": int(entry["era"]),
                "progression": entry["progression"],
                "strategy_profile": entry["treatment"],
                ...
                "setup_json": json.dumps(
                    {
                        ...
                        "player_allocation": entry["player_allocation"],
                    },
                    sort_keys=True,
                    separators=(",", ":"),
                ),
    ...
    status = "COMPLETED" if counts["SUCCESS"] == manifest["planned_run_count"] else "PARTIAL"
    completed_at = datetime.now(timezone.utc)
