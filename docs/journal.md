...
### Context
Up to this point, Pitgun focused on ingesting, transforming, and analyzing telemetry. What was still missing was a formal contract between game logic, physics, and telemetry generation.

In other words:
> Who decides what the car is allowed to do, and how do we guarantee that physics execution is deterministic, reproducible, and non-tamperable?

This question matters not only for games, but for any serious simulation platform:
- motorsport simulators,
- digital twins,
- financial market replays,
- or regulated environments where trust boundaries matter.

### The core problem

Before this work, the flow implicitly assumed that:
- the client knows which parameters it can tune,
- physics code trusts the input blindly,
- telemetry is generated without a verifiable origin.

That model does not scale:
- it is fragile against cheating,
- it makes replay non-deterministic,
- and it prevents any serious server-side validation.

What Pitgun needed was a **cryptographically verifiable simulation contract**.

### Introducing signed simulation contracts

The solution is a signed simulation contract, generated by a trusted authority: `pitgun-configd`.

The contract acts as a single source of truth describing:
- the policy version used,
- the validated player tuning parameters,
- the upgrades and era context,
- the determinism metadata,
- an issue timestamp + expiration,
- and a cryptographic signature (HMAC-SHA256).

From this point on:
> Physics never consumes raw player input.
> It only consumes signed contracts.

### Role of pitgun-configd

`pitgun-configd` is the policy gatekeeper.

Its responsibilities are:
1. Load and validate the tuning policy (`tuning.v1.yaml`)
2. Validate player input (tuning + upgrades + era)
3. Canonicalize parameters into a stable representation
4. Build a `SimulationContractV1`
5. Sign it using a server-side secret
6. Return a `SignedSimulationContractV1` to the client

If the policy changes, the hash changes.
If the contract expires, it is rejected.
If the signature does not match, physics refuses to run.

This establishes a **hard trust boundary**.

### Deterministic physics as a pure function

With contracts in place, the physics engine (`pitgun-source-physics`) could be redesigned properly.

The key rule is simple:
> Physics is a pure function of a signed contract + time.
...
- checks contract expiration,
- maps canonical parameters to a stable internal config,
- and produces deterministic telemetry batches.
...
- and ready to be streamed.

The final pipeline becomes:
```
Player → pitgun-configd → SignedSimulationContract → pitgun-source-physics → Telemetry batches → pitgun-telemetryd (NDJSON, later Parquet)
```
...
- and enables verifiable simulation.
...
- WASM port of the physics engine (client-side, sandboxed)
- WebSocket ingestion of physics telemetry
- Server-side recomputation for anti-cheat
- Deterministic replay validation
- Multi-era / multi-policy coexistence
