...
## 14 – Signed simulation contracts & deterministic physics

### Context
Pitgun is a generic telemetry framework. The physics engine is only one source, but it still needs a formal contract between policy, simulation, and telemetry generation.

In other words:
> Who decides what inputs are allowed, and how do we guarantee deterministic, reproducible, and non-tamperable physics output?

This matters beyond games:
- motorsport simulators,
- digital twins,
- financial replays,
- regulated industrial systems.

We also enforced a strict separation: `pitgun-core` stays source-agnostic, game contracts live under `pitgun-contract::game::v1`, and physics orchestration lives in `pitgun-source-physics`.

### The core problem

Before this work, the flow was informal:
- clients sent ad‑hoc tuning,
- physics trusted the input,
- telemetry had no verifiable origin or expiration.

That model does not scale:
- it is fragile against cheating,
- it allows indefinite replay of old requests,
- and it weakens server-side validation.

What Pitgun needed was a **cryptographically verifiable, expiring simulation contract**.

### Introducing signed game simulation contracts

The solution is a game-scoped contract, generated by a trusted authority: `pitgun-configd`.

The contract acts as a single source of truth describing:
- the validated tuning parameters,
- the track + deterministic metadata,
- issuance and expiration timestamps,
- a nonce for uniqueness,
- and an HMAC-SHA256 signature.

From this point on:
> Physics workflows never consume raw player input.
> They consume a validated request or a signed contract.

Concretely:
- `GameSimulationRequestV1` holds tuning, track_id, hz, seed, engine_version.
- `GameSimulationContractV1` wraps a payload (request + issued/expires + nonce) and a signature.
- Signing bytes are a canonical binary encoding with domain separation to avoid JSON float ambiguity.

### Role of pitgun-configd

`pitgun-configd` is the policy gatekeeper for the game source.

Its responsibilities are:
1. Validate player tuning via `pitgun-policy`
2. Canonicalize the request into `GameSimulationRequestV1`
3. Compute `issued_at_ms` and `expires_at_ms` (TTL via `PITGUN_CONTRACT_TTL_MS`)
4. Generate a nonce
5. Sign the payload using `pitgun-signing`
6. Return `GameSimulationContractV1` to the client

If the contract expires, it is rejected.
If any field is modified, verification fails.

This establishes a **hard trust boundary**.

### Deterministic physics as a pure function

With contracts in place, the physics engine (`pitgun-source-physics`) could be redesigned cleanly.

The key rule is simple:
> Physics is a pure function of the canonical request + track + hz.

No hidden state.
No runtime configuration.
No player-controlled parameters.

The physics source:
- resolves tracks internally (game source only),
- can verify signature + expiration in the adapter/emitters,
- maps canonical parameters to a stable internal config,
- and produces deterministic telemetry + events.

This makes the physics engine:
- replayable,
- testable,
- portable (including WASM),
- and suitable for server-side verification.

### Telemetry generation pipeline

At this stage, telemetry is:
- generated deterministically,
- stored in memory as batches,
- and ready to be streamed over HTTP or WS.

The final pipeline becomes:
```
Player → pitgun-configd → GameSimulationContractV1 → physics emitter → SessionEnvelope batches → pitgun-telemetryd
```
...
- and reproducible replays.

### Why this matters

This is not *just a game feature*.

This architecture:
- mirrors real motorsport homologation workflows,
- matches regulated system design (finance, energy),
- and enables verifiable simulation at the boundary.

A signed contract is:
- a capability,
- a security boundary,
- and a determinism anchor.

Pitgun now has a **formal execution model**, not just a data pipeline.

What’s next

This chapter unlocks several strategic directions:
- WASM-side physics for local simulation
- WebSocket ingestion of physics telemetry
- Server-side recomputation for anti-cheat
- Deterministic replay validation
- Multi-policy coexistence
- Long-term telemetry storage (Parquet)

With signed contracts, Pitgun crosses a line: from telemetry tooling to a simulation platform.
